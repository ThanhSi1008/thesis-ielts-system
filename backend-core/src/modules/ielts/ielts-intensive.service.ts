...
@Injectable()
export class IeltsIntensiveService {
  constructor(
  ...
  async getIntensiveCatalog(params: { userId: string; skill?: string }) {
    ...
    const parsed = exams
      ...
      .filter(
        ...
        meta: NonNullable<ReturnType<IeltsIntensiveService["parseCambridgeTitle"]>>;
      }>;
  ...
  async submitSession(
    ...
    // 3. Update session + create result in parallel where possible
    const now = new Date();
    ...
    if (graded) {
      // Listening / Reading: run session update and result upsert in parallel
      const resultData = {
    ...
    // 4. Writing & Speaking: update session, then publish grading task
  ...
  // ── Results Queries (Merged from ResultsModule) ──────────────────────────

  async findResultByUser(userId: string) {
  ...
  // ── Study Notes (Merged from NotesModule) ────────────────────────────────
