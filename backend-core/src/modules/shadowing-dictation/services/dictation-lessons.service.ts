...
export class DictationLessonsService {
  ...
  async findAll(userId: string) {
    ...
    const cacheKey = "lessons:dictation:all";
    ...
    if (!lessons) {
      lessons = await this.prisma.dictationVideo.findMany({
        where: { userId: null },
        orderBy: { id: "asc" },
      });
  ...
  async findById(id: string) {
    const cacheKey = `lesson:dictation:${id}`;
    let lesson: any = null;
    ...
    if (!lesson) {
      lesson = await this.prisma.dictationVideo.findFirst({
