...
export class CacheInvalidationService {
  ...
  async onModuleInit() {
    try {
      const rabbitUrl = this.configService.get<string>('RABBITMQ_URL') || 'amqp://toeic:toeic_password@localhost:5672';
      this.connection = await amqp.connect(rabbitUrl) as any;
      this.channel = await (this.connection as any).createChannel();

      // Setup exchange and queue
      await this.channel.assertExchange(CACHE_INVALIDATION_EXCHANGE, 'fanout', { durable: true });
      await this.channel.assertQueue(CACHE_INVALIDATION_QUEUE, { durable: true });
      await this.channel.bindQueue(CACHE_INVALIDATION_QUEUE, CACHE_INVALIDATION_EXCHANGE, '');
      ...
      await this.channel.consume(CACHE_INVALIDATION_QUEUE, async (msg) => {
        if (msg) {
          try {
            ...
            this.logger.error('Failed to process cache invalidation event', error);
      ...
      this.logger.error('❌ Failed to connect to RabbitMQ for cache invalidation', error);
    }
  }
  ...
  async publishInvalidation(pattern: string, source: string) {
    ...
    this.logger.debug(`Published cache invalidation: ${pattern} from ${source}`);
  }

  // Handle incoming invalidation events
  private async handleInvalidation(event: CacheInvalidationEvent) {
    this.logger.log(`Invalidating cache: ${event.pattern} (from ${event.source})`);
