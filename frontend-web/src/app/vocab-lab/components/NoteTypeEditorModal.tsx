...
import type { NoteType, NoteTypeField, CardTemplate, FieldStyle, CardStyle } from '@/types';
                        ...
                        <td className="px-4 py-2.5" onClick={e => e.stopPropagation()}>
                          <select
                            ...
                            onChange={async (e) => {
                              ...
                              const updated = await vocabLabApi.updateField(noteType.id, f.id, { fieldType: newType });
...
function CardsTab({ noteType, onClose }: { noteType: NoteType; onClose: () => void }) {
  ...
  useEffect(() => { setTemplates(noteType.templates); }, [noteType.templates]);
                  ...
                  {fields.map(f => (
                    <label key={f.id} className="flex items-center gap-2 cursor-pointer text-[13px] text-gray-700 font-medium group">
                      <input type="checkbox" checked={frontFields.has(f.id)} onChange={() => toggleFront(f.id)}
                        disabled={noteType.isBuiltIn} className="accent-primary w-4 h-4 cursor-pointer" />
