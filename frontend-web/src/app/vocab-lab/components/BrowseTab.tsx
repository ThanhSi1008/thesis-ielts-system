          ...
          <div>
            <div className="flex items-center gap-1.5 mb-3">
              ...
              <button
                ...
                className={`flex items-center text-xs uppercase tracking-wider font-bold transition-colors ${filter?.type === 'noteType' && filter.value === '__all' ? 'text-[#D97706]' : 'text-gray-500 hover:text-gray-800'
                  }`}
              >
            ...
            <div className="space-y-4 flex-1">
              {[...(selectedCard.noteType?.fields ?? [])].sort((a, b) => a.order - b.order).map((field, idx) => {
                ...
                const fieldStyle = fieldStyleToCSS({
                  ...(selectedCard.noteType?.templates?.[0]?.fieldStyles?.[field.id] as any || {}),
