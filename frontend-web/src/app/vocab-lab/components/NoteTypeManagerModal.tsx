...
import { NoteTypeEditorModal } from './NoteTypeEditorModal';

interface Props { onClose: () => void; }

type Dialog =
  | { kind: 'rename'; nt: NoteType; value: string }
  | { kind: 'delete'; nt: NoteType };
...
export function NoteTypeManagerModal({ onClose }: Props) {
  ...
  const handleAdd = async () => {
    ...
    if (!name) return;
    const created = await vocabLabApi.createNoteType(name);
  ...
  const confirmRename = async () => {
    ...
    try {
      await vocabLabApi.renameNoteType(dialog.nt.id, name);
      await load();
      setDialog(null);
  ...
  const confirmDelete = async () => {
    ...
    setActionLoading(true);
    try {
      await vocabLabApi.deleteNoteType(dialog.nt.id);
      await load();
      setSelectedId('');
      setDialog(null);
    } finally { setActionLoading(false); }
  ...
  const editTarget = noteTypes.find(nt => nt.id === editTargetId);
  ...
  return (
    <>
              ...
              <div>
                ...
                {(dialog.nt.cardCount ?? 0) > 0 ? (
                  <p className="mt-1 text-[12.5px] text-gray-500 leading-relaxed">
                    ...
                    <span className="font-semibold text-red-500">{dialog.nt.cardCount} card{dialog.nt.cardCount === 1 ? '' : 's'}</span>{' '}
      ...
      {editTargetId && editTarget && (
        <NoteTypeEditorModal noteType={editTarget} onClose={() => { setEditTargetId(null); load(); }} />
      )}
    </>
  );
}
