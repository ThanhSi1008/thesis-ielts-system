...
import type { DictationFormData, DictationSentenceFormData } from "../_hooks/useAdminDictationForm";
import { DictationSentenceEditor } from "./DictationSentenceEditor";

const CATEGORIES = ["Conversation", "TED Talk", "Movie Clip", "Music", "News", "Other"] as const;

interface DictationLessonFormProps {
  formData: DictationFormData;
  errors: Partial<Record<keyof DictationFormData, string>>;
  isSubmitting: boolean;
  submitLabel: string;
  onSetField: <K extends keyof DictationFormData>(field: K, value: DictationFormData[K]) => void;
  onAddSentence: () => void;
  onRemoveSentence: (index: number) => void;
  onUpdateSentence: (index: number, updates: Partial<DictationSentenceFormData>) => void;
        ...
        <div>
          ...
          {formData.imageUrl && (
            <img
              ...
              className="mt-2 h-20 w-auto rounded-lg object-cover border border-gray-200 dark:border-gray-700"
            />
          )}
        </div>
      </div>
