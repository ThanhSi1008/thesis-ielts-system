...
import { adminIeltsAdvancedApi, ieltsImportApi } from "@/services/admin.api";
...
type AdvancedTab = "listening" | "reading" | "writing" | "speaking" | "staging";

export default function IELTSAdvancedAdminPage() {
  const [activeTab, setActiveTab] = useState<AdvancedTab>("listening");

  // Advanced Lists State
  ...
  const fetchAdvancedLists = useCallback(async () => {
    ...
    try {
      ...
      setListeningParts(listening);
      setReadingParts(reading);
      setWritingPrompts(writing);
      setSpeakingParts(speaking);
    } catch {
      toast.error("Failed to load advanced practice parts.");
    } finally {
  ...
  useEffect(() => {
    ...
    fetchStagingQueue();
    return () => stopPolling();
  }, [fetchAdvancedLists, fetchStagingQueue, stopPolling]);

  // ─── Live Actions ───
  const handleTogglePublish = async (id: string, skillType: "listening" | "reading" | "writing" | "speaking", currentStatus: boolean) => {
    try {
      if (skillType === "listening") {
        await adminIeltsAdvancedApi.publishListening(id);
        setListeningParts(prev => prev.map(p => p.id === id ? { ...p, isPublished: !p.isPublished } : p));
      } else if (skillType === "reading") {
        await adminIeltsAdvancedApi.publishReading(id);
        setReadingParts(prev => prev.map(p => p.id === id ? { ...p, isPublished: !p.isPublished } : p));
      } else if (skillType === "writing") {
        await adminIeltsAdvancedApi.publishWriting(id);
        setWritingPrompts(prev => prev.map(p => p.id === id ? { ...p, isPublished: !p.isPublished } : p));
      } else if (skillType === "speaking") {
        await adminIeltsAdvancedApi.publishSpeaking(id);
        setSpeakingParts(prev => prev.map(p => p.id === id ? { ...p, isPublished: !p.isPublished } : p));
      }
      toast.success(currentStatus ? "Part drafted." : "Part published successfully!");
    } catch {
      toast.error("Failed to update publish status.");
    }
  };

  const handleDeletePart = async (id: string, skillType: "listening" | "reading" | "writing" | "speaking") => {
    if (!confirm("Are you sure you want to delete this live advanced part? This cannot be undone.")) return;
    try {
      if (skillType === "listening") {
        await adminIeltsAdvancedApi.deleteListening(id);
        setListeningParts(prev => prev.filter(p => p.id !== id));
      } else if (skillType === "reading") {
        await adminIeltsAdvancedApi.deleteReading(id);
        setReadingParts(prev => prev.filter(p => p.id !== id));
      } else if (skillType === "writing") {
        await adminIeltsAdvancedApi.deleteWriting(id);
        setWritingPrompts(prev => prev.filter(p => p.id !== id));
      } else if (skillType === "speaking") {
        await adminIeltsAdvancedApi.deleteSpeaking(id);
        setSpeakingParts(prev => prev.filter(p => p.id !== id));
      }
  ...
  // ─── Helper: Render Table ───
  const renderListTable = (items: any[], type: "listening" | "reading" | "writing" | "speaking") => {
    if (isListLoading) {
      return (
    ...
    if (items.length === 0) {
      return (
        <div className="text-center py-20 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-3xl bg-white dark:bg-gray-900">
          <p className="text-gray-500 dark:text-gray-400 text-sm font-medium">No live advanced {type} parts found.</p>
        </div>
      );
    }
    ...
    return (
      <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full text-sm">
          <thead className="border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-950/20">
            <tr>
              <th className="text-left px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-gray-400 font-bold">Details</th>
              <th className="text-left px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-gray-400 font-bold">Provenance</th>
              <th className="text-left px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-gray-400 font-bold">Status</th>
              <th className="text-right px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-gray-400 font-bold">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
            {items.map(item => (
              <tr key={item.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-850/20 transition-colors">
                <td className="px-5 py-4">
                  <div className="font-semibold text-gray-900 dark:text-gray-100 max-w-[360px] truncate">{item.title}</div>
                  <div className="text-[10px] text-gray-400 select-all truncate mt-0.5 max-w-[360px]">{item.id}</div>
                </td>
                <td className="px-5 py-4 text-gray-600 dark:text-gray-400 capitalize">
                  {item.source} {item.bookNumber && `(Book ${item.bookNumber} - Test ${item.testNumber} - Part ${item.partNumber || 1})`}
                </td>
                <td className="px-5 py-4">
                  <button
                    onClick={() => handleTogglePublish(item.id, type, item.isPublished)}
                ...
                <td className="px-5 py-4 text-right">
                  <button
                    onClick={() => handleDeletePart(item.id, type)}
  ...
  return (
    <div className="p-6 max-w-6xl">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">IELTS Advanced Practice Parts</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Manage focused advanced practice parts (Listening, Reading, Writing, Speaking) and importing pipelines.
          </p>
        </div>
        <button
      ...
      <div className="flex items-center gap-2 border-b border-gray-100 dark:border-gray-800 mb-6 pb-0.5">
        {(["listening", "reading", "writing", "speaking"] as const).map(tab => (
          <button
            ...
            className={`px-4 py-2.5 text-sm font-semibold border-b-2 capitalize transition-colors -mb-0.5 ${
              activeTab === tab
      ...
      {/* Live Lists Panels */}
      {activeTab === "listening" && renderListTable(listeningParts, "listening")}
      {activeTab === "reading" && renderListTable(readingParts, "reading")}
      {activeTab === "writing" && renderListTable(writingPrompts, "writing")}
      {activeTab === "speaking" && renderListTable(speakingParts, "speaking")}
        ...
        isStagingLoading ? (
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm">
            <table className="w-full text-sm">
              <thead className="border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-950/20">
                <tr>
                  <th className="text-left px-5 py-3 text-xs font-bold uppercase tracking-wider text-gray-400 font-bold">Skill</th>
                  <th className="text-left px-5 py-3 text-xs font-bold uppercase tracking-wider text-gray-400 font-bold">Reference / Title</th>
                  <th className="text-left px-5 py-3 text-xs font-bold uppercase tracking-wider text-gray-400 font-bold">Created At</th>
                  <th className="text-left px-5 py-3 text-xs font-bold uppercase tracking-wider text-gray-400 font-bold">Status</th>
                  <th className="text-right px-5 py-3 text-xs font-bold uppercase tracking-wider text-gray-400 font-bold">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                {jobs.map(job => (
      ...
      {showImportDrawer && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md transition-all duration-300 animate-fade-in">
          ...
          <div className="relative w-full max-w-4xl bg-white dark:bg-gray-900 border border-gray-150 dark:border-gray-800 shadow-2xl rounded-3xl flex flex-col max-h-[90vh] overflow-hidden animate-scale-up">
            ...
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 px-6 py-4 shrink-0 bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">Scrape web text/PDFs to advanced focused database bank</p>
                </div>
              </div>
              <button
                onClick={() => setShowImportDrawer(false)}
                className="p-1.5 text-gray-400 hover:text-gray-700 dark:hover:text-gray-250 hover:bg-gray-55 dark:hover:bg-gray-850 rounded-xl transition-all"
              >
            ...
            {/* Info Banner */}
            <div className="mx-6 mt-4 shrink-0 bg-primary/5 dark:bg-primary/10 border border-primary/20 rounded-2xl p-4 text-[11px] text-primary-700 dark:text-primary-400 leading-relaxed flex items-start gap-2.5">
            ...
            <form onSubmit={handleCreateImportJob} className="flex flex-col flex-1 overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                  ...
                  <div className="flex flex-col gap-5">
                    <div>
                      <label className="block text-xs font-bold text-gray-655 dark:text-gray-400 mb-1.5">Target Skill *</label>
                      <select
                        value={skill}
                  ...
                  <div className="flex flex-col gap-5">
                      ...
                      <div className="border border-gray-150 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-900/30 rounded-2xl p-4 flex flex-col gap-3 animate-fade-in">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {[1, 2, 3, 4].map(partNum => {
                            ...
                            return (
                              <div key={partNum} className="bg-white dark:bg-gray-900 border border-gray-150/70 dark:border-gray-805 p-2.5 rounded-xl flex flex-col justify-between shadow-sm relative overflow-hidden">
                                <div className="flex items-center justify-between">
                                ...
                                {uploadedUrl ? (
                                  ...
                                  <div className="mt-2.5">
                                    ...
                                    <label
                                      ...
                                      className="block w-full text-center py-1 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-750 border border-gray-200 dark:border-gray-700 rounded-lg text-[9px] font-bold text-gray-655 dark:text-gray-300 cursor-pointer transition-colors"
                                    >
                    ...
                    {skill === "LISTENING" && (
                      <div className="border border-gray-150 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-900/30 rounded-2xl p-4 flex flex-col gap-3 animate-fade-in">
                        <label className="block text-xs font-bold text-gray-655 dark:text-gray-400">Official Answer Key Image</label>
                        {answerKeyImage ? (
                          <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-855 p-3 rounded-xl flex items-center justify-between shadow-sm relative overflow-hidden">
                            ...
                            <button
                              ...
                              className="text-[10px] font-bold text-red-500 hover:text-red-655 shrink-0"
                            >
                              Remove
                            </button>
                          </div>
                            ...
                            <label
                              ...
                              className={[
                                ...
                                isDragActiveImage 
                                  ...
                                  : "border-gray-300 dark:border-gray-750 text-gray-700 dark:text-gray-250"
                              ].join(" ")}
                            >
                              {isUploadingImage ? (
              ...
              <div className="flex gap-3 px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50 justify-end shrink-0">
                ...
                <button
                  type="submit"
                  disabled={isSubmittingJob || isUploading || isUploadingImage || uploadingAudioPart !== null || (skill === "LISTENING" && audioUrls.filter(Boolean).length < 4)}
