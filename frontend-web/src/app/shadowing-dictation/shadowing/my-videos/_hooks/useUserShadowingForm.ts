...
export function useUserShadowingForm(initialData?: ShadowingVideo) {
  ...
  const submitUpdate = useCallback(async (videoId: string): Promise<ShadowingVideo | null> => {
    if (!validate()) return null;
    setIsSubmitting(true);
    try {
      return await shadowingApi.updateVideo(videoId, buildPayload());
    } finally {
