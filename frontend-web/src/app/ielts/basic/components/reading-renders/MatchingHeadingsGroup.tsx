...
export interface MatchingHeadingsGroup {
  type: 'matching_headings';
  instruction?: string;
  options: HeadingOption[];
  questions: HeadingQuestion[];
}
...
export function MatchingHeadingsGroup({
  group,
  answers,
  onAnswer,
  submitted,
  ...
  return (
    <div className="mb-8 font-sans">
      ...
      <p className="text-[13px] text-gray-600 mb-6 leading-relaxed">
        {group.instruction || 'Choose the correct heading for each paragraph.'}
      </p>
        ...
        <table className="w-full text-left border-collapse min-w-[500px]">
          ...
          <tbody>
            {group.questions.map((q, idx) => {
              ...
              const isCorrect = submitted && selected.toLowerCase() === q.answer.toLowerCase();
              ...
              return (
                <tr key={idx} className={`border-b border-gray-200 last:border-0 hover:bg-gray-50/50 transition-colors ${
                  ...
                  {optionLetters.map(letter => {
                    ...
                    const isCorrectCell = submitted && letter.toLowerCase() === q.answer.toLowerCase();
      ...
      {options.length > 0 && (
        <div className="mb-8 overflow-hidden border border-gray-300 rounded-lg shadow-sm">
          ...
          <div className="bg-white">
            {options.map((opt, idx) => (
              <div key={idx} className="flex border-b border-gray-200 last:border-0 hover:bg-gray-50 transition-colors">
                <div className="w-12 px-4 py-3 flex items-center justify-center border-r border-gray-200 bg-gray-50/30 font-bold text-gray-700 text-[13px]">
                  {opt.letter}
                </div>
