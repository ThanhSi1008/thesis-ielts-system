...
export default function IntensiveTestTakePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([res, session]) => {
        ...
        setSecondsLeft(Math.floor((res.duration * 60) / 4));
  ...
  const parts = useMemo(() => {
     const arr = (exam?.questions?.parts as any[]) || [];
     if (!sessionInfo?.practicePart) return arr;
     return arr.filter(p => (p.part_number || p.passage_number || p.task_number || 1) === sessionInfo.practicePart);
  }, [exam, sessionInfo]);
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-10 max-w-md w-full flex flex-col items-center">
            ...
            <div className="flex gap-8 mb-8 w-full justify-center">
              <div className="flex flex-col items-center">
                <div className="text-4xl font-black text-[#D51025]">{submitResult.totalScore}/{qNumbers.length}</div>
                <div className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-2">Correct</div>
              </div>
            </div>
