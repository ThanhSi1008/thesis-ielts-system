...
export default function PracticeReadingBoard({
  ...
  const handleSaveAndExit = async () => {
    ...
    try {
      ...
      const answersWithMeta = {
        ...answers,
        _meta: {
          customTime: searchParams?.get("customTime"),
          autoSubmit: searchParams?.get("autoSubmit"),
          partIndex: searchParams?.get("partIndex"),
          secondsLeft
        }
      };
      await examsApi.saveSessionProgress(sessionInfo.id, answersWithMeta, timeTaken);
  ...
  return (
    <div className="h-screen flex flex-col font-sans bg-[#F3F4F6] overflow-hidden text-[#1A1A1A]">
      <header className="h-[60px] flex-shrink-0 bg-white border-b border-gray-300 flex items-center justify-between px-6 z-20 shadow-[0_2px_4px_rgba(0,0,0,0.02)]">
        ...
        <div className="flex items-center gap-6 text-gray-700">
          ...
          <button
            ...
            disabled={submitting || submitResult || saving}
            className="ml-2 px-4 py-1.5 text-[13px] font-black rounded border border-gray-300 hover:bg-gray-50 text-gray-700 transition-all active:scale-95 disabled:opacity-60 uppercase"
          >
      ...
      <main className="flex-1 min-h-0 bg-[#f5f5f5] shadow-inner relative flex overflow-hidden">
      ...
      <footer className="h-[60px] flex-shrink-0 bg-[#f8f9fa] z-20 flex items-center w-full shadow-[0_-2px_4px_rgba(0,0,0,0.02)]">
        <div className="flex-1 flex items-center h-full gap-6 overflow-x-auto custom-scrollbar px-6">
          ...
          <div className="flex h-full items-center min-w-max relative px-4 transition-colors">
            <div className="flex items-center h-full">
              <div className="relative h-full flex items-center pr-3">
                <div className={`absolute top-0 left-0 right-3 h-[3px] ${answeredSet.size === qNumbers.length && qNumbers.length > 0 ? 'bg-[#319c28]' : 'bg-[#dcdcdc]'}`} />
