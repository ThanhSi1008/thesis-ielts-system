...
export default function Header() {
  ...
  // The rest of the IELTS section (except dashboard) uses the dark theme
  ...
  // On all other pages (like homepage) the nav overlays the PageHeader hero with white text
  ...
  const headerBgClass = isIeltsDashboard
    ...
    : isIelts 
    ? "bg-dark text-light top-0 border-dark" 
    : (isPlain ? "bg-white top-0 border-gray-100" : "bg-transparent absolute w-full top-0 border-transparent shadow-none");
      ...
      <div className="container mx-auto max-w-screen-xl px-4 py-4 flex justify-between items-center">
        <div className="flex items-center gap-6">
          ...
          <nav className="hidden md:flex gap-10 font-bold items-center justify-center flex-1 ml-10">
            <Link href="/" className={`relative text-sm uppercase tracking-wider transition-colors pt-2 pb-1 group ${pathname === '/' ? 'text-primary' : (isOverlay || isIelts ? 'text-light' : 'text-gray-600')}`}>
              HOME
              <span className="absolute left-0 bottom-0 h-[2px] bg-primary transition-all duration-300 w-0 group-hover:w-full"></span>
            </Link>
            <Link href="/ielts" className={`relative text-sm uppercase tracking-wider transition-colors pt-2 pb-1 group ${pathname === '/ielts' || pathname.startsWith('/ielts/') ? 'text-primary' : (isOverlay || isIelts ? 'text-light' : 'text-gray-600')}`}>
              IELTS
              <span className="absolute left-0 bottom-0 h-[2px] bg-primary transition-all duration-300 w-0 group-hover:w-full"></span>
            </Link>
            <Link href="/vocab-lab" className={`relative text-sm uppercase tracking-wider transition-colors pt-2 pb-1 group ${(pathname === '/vocab-lab' || pathname.startsWith('/vocab-lab/')) ? 'text-primary' : (isOverlay || isIelts ? 'text-light' : 'text-gray-600')}`}>
              VOCAB LAB
              <span className="absolute left-0 bottom-0 h-[2px] bg-primary transition-all duration-300 w-0 group-hover:w-full"></span>
            </Link>
            <Link href="/shadowing-dictation" className={`relative text-sm uppercase tracking-wider transition-colors pt-2 pb-1 group ${(pathname === '/shadowing-dictation' || pathname.startsWith('/shadowing-dictation/')) ? 'text-primary' : (isOverlay || isIelts ? 'text-light' : 'text-gray-600')}`}>
              SHADOWING & DICTATION
              <span className="absolute left-0 bottom-0 h-[2px] bg-primary transition-all duration-300 w-0 group-hover:w-full"></span>
            </Link>
          </nav>
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          {user ? (
            <div className="flex items-center gap-4">
              <span className={`text-sm font-semibold ${isOverlay || isIelts ? 'text-light' : 'text-gray-900'}`}>Hello, {user.firstName && user.lastName ? `${user.firstName} ${user.lastName}` : user.email}</span>
              <button
                onClick={logout}
                className={`${isIelts ? 'bg-light/10 hover:bg-light/20' : 'bg-white/20 hover:bg-white/30'} text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors`}
              >
                Logout
              </button>
            </div>
            ...
            <Link href="/login" className="flex items-center gap-2 group">
              <img src="https://res.cloudinary.com/dalaaegob/image/upload/v1772879077/ae371d4a-9b66-4a55-abe6-06695c9f6986.png" alt="Sign In" className={`h-8 w-8 object-contain ${isIelts ? 'contrast-200 grayscale brightness-200' : ''}`} />
              <span className={`font-semibold text-sm hover:underline ${isOverlay || isIelts ? 'text-light' : 'text-gray-700'}`}>Sign In</span>
            </Link>
          )}
        </div>
        ...
        {/* Mobile Menu Button */}
        <button
          className={`md:hidden p-2 ${isOverlay || isIelts ? 'text-light' : 'text-gray-600'}`}
