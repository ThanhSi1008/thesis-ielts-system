...
import type { IeltsIntensiveExam, IeltsIntensiveSession, IeltsIntensiveCatalogResponse, PracticeCatalogResponse, IeltsBasicSkill } from '@/types';
...
export const examsApi = {
  ...
  getExam: async (id: string) => {
    const { data } = await api.get<IeltsIntensiveExam>(`/exams/${encodeURIComponent(id)}`);
    return data;
  },
  createSession: async (examId: string, userId: string, practicePart?: number) => {
    const { data } = await api.post<IeltsIntensiveSession>(`/exams/${encodeURIComponent(examId)}/sessions`, { userId, practicePart });
    return data;
  },
  getSession: async (sessionId: string) => {
    const { data } = await api.get<IeltsIntensiveSession & { ieltsIntensiveExam: any; ieltsIntensiveResult: any }>(`/exams/sessions/${encodeURIComponent(sessionId)}`);
    return data;
  },
  submitSession: async (sessionId: string, answers: Record<string, string | number | string[]>, timeTaken?: number) => {
    const { data } = await api.post<IeltsIntensiveSession>(`/exams/sessions/${encodeURIComponent(sessionId)}/submit`, { answers, timeTaken });
    return data;
  },
  ...
  saveSessionProgress: async (sessionId: string, answers: Record<string, any>, timeTaken?: number) => {
    const { data } = await api.patch<IeltsIntensiveSession>(`/exams/sessions/${encodeURIComponent(sessionId)}/progress`, { answers, timeTaken });
    return data;
  },
