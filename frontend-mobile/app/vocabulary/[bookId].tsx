...
import { vocabularyApi, BookWithUnits } from '../../services/api';

export default function VocabularyBookScreen() {
  const { bookId } = useLocalSearchParams<{ bookId: string }>();
  const [book, setBook] = useState<BookWithUnits | null>(null);
  ...
  const loadBook = async () => {
    try {
      setLoading(true);
      const data = await vocabularyApi.getBook(bookId!);
      setBook(data);
    } catch (err) {
      console.error(err);
    } finally {
    ...
    <>
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={styles.list}>
          {book.units.map((unit) => (
            <Link key={unit.id} href={`/vocabulary/${bookId}/${unit.id}`} asChild>
