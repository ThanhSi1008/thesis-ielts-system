/**
 * IELTS Pronunciation Detail — /ielts/pronunciation/[symbol]
 * Same AI recorder + scoring as (tabs)/pronunciation/[symbol]
 * Back button navigates to /ielts/pronunciation (IELTS context)
 */
...
// ─── Example words (same as tabs version) ────────────────────────────────────
...
function useExponentialPoller() {
  ...
  const startPolling = useCallback(
    (fn: () => Promise<boolean>, opts: { maxMs?: number } = {}) => {
      ...
      let delay = 1_000;
      const tick = async () => {
        if (Date.now() - start >= maxMs) { fn(); return; }
        const done = await fn();
        if (!done) {
...
function WordCard({ word, sentence, index, userId }: {
  word: string; sentence: string; index: number; userId: string;
}) {
  ...
  const handleToggle = useCallback(async () => {
    if (status === 'recording' || isRecording) {
        ...
        startPolling(async () => {
          try {
            ...
            if (attempt?.status === 'COMPLETED') {
              ...
              setStatus('done');
              if (pronScore >= 80) Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      ...
      setStatus('recording'); setScore(null); setFeedback(null); setErrMsg(null);
      await startRecording();
    }
  }, [status, isRecording, word, userId, startRecording, stopRecording, startPolling]);

  const reset = useCallback(() => {
    stopPolling(); setStatus('idle'); setScore(null); setFeedback(null); setErrMsg(null);
  }, [stopPolling]);
      ...
      <View style={styles.recorderSide}>
        {status === 'processing' ? (
          ...
          <View style={styles.errorBox}>
            ...
            <TouchableOpacity onPress={reset}><Text style={styles.retryText}>Retry</Text></TouchableOpacity>
          </View>
        ) : (
          <TouchableOpacity
...
export default function IeltsSoundDetailScreen() {
  ...
  const typeLabel = typeKey === 'monophthong'
    ...
    : typeKey === 'diphthong' ? 'Diphthong · Gliding Vowel' : 'Consonant';
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Animated.View entering={FadeIn} style={[styles.symbolBadge, { backgroundColor: colors.bg }]}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {user ? (
          examples.map((ex, i) => (
            <WordCard key={ex.word} word={ex.word} sentence={ex.sentence} index={i} userId={user.id} />
          ))
        ) : (
...
const styles = StyleSheet.create({
  ...
  symbolBadge: { width: 56, height: 56, borderRadius: RADIUS.lg, alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 3 },
  ...
  tipBanner: { flexDirection: 'row', alignItems: 'flex-start', gap: SPACING.sm, backgroundColor: COLORS.primary + '12', margin: SPACING.lg, marginBottom: 0, padding: SPACING.md, borderRadius: RADIUS.lg, borderWidth: 1, borderColor: COLORS.primary + '25' },
  tipText: { flex: 1, fontSize: FONT_SIZES.sm, color: COLORS.text, lineHeight: 18, fontFamily: FONTS.regular },
  scroll: { flex: 1 },
  scrollContent: { padding: SPACING.lg },
  sectionLabel: { fontSize: FONT_SIZES.xs, fontFamily: FONTS.bold, color: COLORS.textMuted, letterSpacing: 1, textTransform: 'uppercase', marginBottom: SPACING.md },
  wordCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.card, borderRadius: RADIUS.xl, padding: SPACING.lg, marginBottom: SPACING.md, borderWidth: 1, borderColor: 'rgba(0,0,0,0.04)', shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 8, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
  ...
  micBtn: { width: 48, height: 48, borderRadius: 24, backgroundColor: COLORS.primary + '15', alignItems: 'center', justifyContent: 'center', borderWidth: 1.5, borderColor: COLORS.primary + '40' },
  micBtnActive: { backgroundColor: '#EF4444', borderColor: '#DC2626' },
  recordingDot: { position: 'absolute', top: 6, right: 6, width: 8, height: 8, borderRadius: 4, backgroundColor: '#fff' },
