...
const IPA_DATA = {
  ...
  consonants: [
    ...
    { symbol: 'ʃ', word: 'shall',voiced: false }, { symbol: 'ʒ', word: 'tv',   voiced: true },
...
// ─── Symbol Cell ──────────────────────────────────────────────────────────────
function SymbolCell({ symbol, word, bg, textColor = '#1a1a2e', onPress }: {
  symbol: string; word: string; bg: string; textColor?: string; onPress: () => void;
}) {
  return (
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.introBanner}>
        ...
        <Text style={styles.introText}>
          Tap any symbol to practice with AI pronunciation scoring. Each symbol has example words and sentence drills.
        </Text>
      </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.grid}>
            {IPA_DATA.monophthongs.map(item => (
              <SymbolCell
                key={item.symbol} symbol={item.symbol} word={item.word}
                bg="#FEF08A" onPress={() => go(item.symbol)}
              />
            ))}
          </View>
          ...
          <View style={styles.grid}>
            {IPA_DATA.diphthongs.map(item => (
              <SymbolCell
                key={item.symbol} symbol={item.symbol} word={item.word}
                bg="#FCA5A5" textColor="#7f1d1d" onPress={() => go(item.symbol)}
              />
            ))}
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.grid}>
            {IPA_DATA.consonants.map(item => (
              <SymbolCell
                key={item.symbol} symbol={item.symbol} word={item.word}
                bg={item.voiced ? '#fff' : '#F3F4F6'} onPress={() => go(item.symbol)}
              />
            ))}
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  introBanner: {
    ...
    backgroundColor: COLORS.primary + '0D', margin: SPACING.lg, marginBottom: 0,
  ...
  sectionTitle: {
    fontSize: FONT_SIZES.md, fontFamily: FONTS.bold, color: COLORS.text,
    textTransform: 'uppercase', letterSpacing: 1, marginBottom: SPACING.sm,
    paddingBottom: SPACING.sm, borderBottomWidth: 2, borderColor: COLORS.primary,
  },
  groupLabel: {
  ...
  cell: {
    width: '21%', aspectRatio: 1, borderRadius: RADIUS.lg,
  ...
  cellWord: {
    fontSize: 9, fontFamily: FONTS.semibold, textTransform: 'uppercase',
    letterSpacing: 0.5, marginTop: 2, opacity: 0.75,
  },
