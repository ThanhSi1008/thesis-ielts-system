...
export function normalizeOptions(opts: any): Record<string, string> {
...
export function normalizePart(rawPart: any): any {
  ...
  if (Array.isArray(part.question_groups)) {
    part.question_groups = part.question_groups.map((g: any) => {
      ...
      if (lowerType.includes('multiple') || lowerType.includes('choice')) {
        ...
        lowerType.includes('yes') ||
        lowerType === 'no' ||
        lowerType.includes('yes/no') ||
        lowerType.includes('yes_no') ||
        lowerType.includes('yes no')
      ) {
      ...
      if (
        (originalType.includes('note') ||
          originalType.includes('form') ||
          originalType.includes('sentence')) &&
        !g.content
      ) {
  ...
  const parseContentList = (contentArr: any[]) => {
    for (const block of contentArr) {
      ...
      if (Array.isArray(subs)) {
        for (const sub of subs) {
          ...
          for (const p of sub?.points ?? []) {
            if (typeof p?.question_number === 'number') {
              ...
              if (!blockHeadingAssigned && heading) {
                itemProps.heading = heading;
                blockHeadingAssigned = true;
              }
              if (!subHeadingAssigned && subheading) {
                itemProps.subheading = subheading;
                subHeadingAssigned = true;
              }
              if (precedingText.length > 0) {
                itemProps.precedingText = precedingText;
                precedingText = [];
              }
        ...
        const allPointsLackText = block.points.every(
          (p: any) => typeof p?.question_number === 'number' && !p.text,
        );

        if (hasBlockText && allPointsLackText) {
          const qns = block.points
            .map((p: any) => p.question_number)
    ...
    for (const g of part.question_groups) {
      ...
      if (!parsedByContent) {
        ...
        if (qt.includes('multiple choice') && Array.isArray(g?.items)) {
          let isFirst = true;
          const groupQns = g.items
            .map(
              (i: any) =>
                i.question_number ||
                (Array.isArray(i.question_numbers) ? i.question_numbers[0] : null),
            )
            .flat()
            .filter(Boolean);
          ...
          (qt.includes('true/false/not given') || qt.includes('yes/no/not given')) &&
          ...
          if (Object.keys(options).length === 0) {
            ...
            if (Object.keys(options).length === 0 && g.items.length > 0) {
              ...
              if (
                firstItemOpts &&
                typeof firstItemOpts === 'object' &&
                !Array.isArray(firstItemOpts)
              ) {
          ...
          for (const it of g.items) {
            if (typeof it?.question_number === 'number') {
              ...
              const trailing =
                trailingMatch && (!leadingMatch || txt.length > leadingMatch[1].length)
                  ? trailingMatch[1]
                  : '';
              ...
              if (blankMatch) {
                ...
                middle =
                  middle.substring(0, idx) +
                  ` ${qNum} [blank] ` +
                  middle.substring(idx + blankMatch[0].length);
                replaced = true;
              }
      ...
      if (groupItems.length > 0) {
        ...
        const allQns: number[] = groupItems
          .flatMap((it: any) => (it.qn ? [it.qn] : it.qns || []))
        ...
        for (let i = groupStartIdx; i < items.length; i++) {
          if (
            ['summary_completion', 'note_completion', 'table_completion'].includes(items[i].kind)
          ) {
