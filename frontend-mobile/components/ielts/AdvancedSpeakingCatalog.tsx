...
export function AdvancedSpeakingCatalog({ embedded = false }: { embedded?: boolean }) {
  ...
  if (embedded) {
    return (
      <View style={{ flex: 1 }}>
        {/* Render a Retest Mic button directly in the catalog list header or search bar if embedded */}
        <View style={{ paddingHorizontal: SPACING.lg, paddingTop: SPACING.sm, flexDirection: 'row', justifyContent: 'flex-end' }}>
          <TouchableOpacity
            style={{ flexDirection: 'row', alignItems: 'center', gap: 4, paddingVertical: 4 }}
            onPress={handleRetestDevices}
          >
            <Ionicons name="construct-outline" size={13} color={colors.primary} />
            <Text style={{ fontSize: 11, fontFamily: FONTS.bold, color: colors.primary }} allowFontScaling={true}>Retest Devices</Text>
          </TouchableOpacity>
        </View>
        {body}
      </View>
