  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([d, ct, savedTags, savedDeckId, savedTypeId]) => {
        ...
        if (savedTags) {
          try {
            setTagsList(JSON.parse(savedTags));
          } catch {}
        }
        ...
        if (d.length > 0) {
          initialDeckId =
            savedDeckId && d.some((x: any) => x.id === savedDeckId) ? savedDeckId : d[0].id;
          setDeckId(initialDeckId);
        }

        if (ct.length > 0) {
          const defaultType =
            (savedTypeId && ct.find((t: any) => t.id === savedTypeId)) ||
  ...
  const uploadMedia = async (type: 'image' | 'audio') => {
    ...
    try {
      ...
      if (!result.canceled && result.assets?.length > 0) {
        ...
        const res = await vocabLabApi.uploadMedia(
          asset.uri,
  ...
  const handleSubmit = async () => {
    ...
    try {
      await vocabLabApi.createFlashcard({
        ...
        cardTypeId,
        fieldValues,
        fieldStyles: Object.keys(fieldStyles).some((k) => Object.keys(fieldStyles[k]).length > 0)
          ? fieldStyles
          : undefined,
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.toolbar}>
          <TouchableOpacity
            style={[s.toolbarBtn, isActiveStyle('fontWeight', 'bold') && s.toolbarBtnActive]}
            onPress={() => toggleStyle('fontWeight', 'bold')}
          >
            <Ionicons
              name="text"
              size={18}
              color={isActiveStyle('fontWeight', 'bold') ? COLORS.primary : COLORS.textMuted}
            />
          </TouchableOpacity>
          <TouchableOpacity
            style={[s.toolbarBtn, isActiveStyle('fontStyle', 'italic') && s.toolbarBtnActive]}
            onPress={() => toggleStyle('fontStyle', 'italic')}
          >
            <Ionicons
              name="text-outline"
              size={18}
              color={isActiveStyle('fontStyle', 'italic') ? COLORS.primary : COLORS.textMuted}
            />
          </TouchableOpacity>
          <View style={s.toolbarDivider} />
          <TouchableOpacity
            style={[s.toolbarBtn, isActiveStyle('textAlign', 'left') && s.toolbarBtnActive]}
            onPress={() => toggleStyle('textAlign', 'left')}
          >
            <Ionicons
              name="menu"
              size={18}
              color={isActiveStyle('textAlign', 'left') ? COLORS.primary : COLORS.textMuted}
            />
          </TouchableOpacity>
          <TouchableOpacity
            style={[s.toolbarBtn, isActiveStyle('textAlign', 'center') && s.toolbarBtnActive]}
            onPress={() => toggleStyle('textAlign', 'center')}
          >
            <Ionicons
              name="menu-outline"
              size={18}
              color={isActiveStyle('textAlign', 'center') ? COLORS.primary : COLORS.textMuted}
            />
          </TouchableOpacity>
          <View style={s.toolbarDivider} />
          <TouchableOpacity
          ...
          >
            <Ionicons
              name="image-outline"
              size={18}
              color={isUploading ? COLORS.border : COLORS.textMuted}
            />
          </TouchableOpacity>
          <TouchableOpacity
            style={s.toolbarBtn}
            onPress={() => uploadMedia('audio')}
            disabled={isUploading}
          >
            <Ionicons
              name="mic-outline"
                      ...
                      return (
                        <View key={mIdx} style={s.mediaPreview}>
                          ...
                          <TouchableOpacity
                            onPress={() => {
                              const remaining = [
                                ...val.matchAll(/<(img|audio)[^>]*>(<\/audio>)?/gi),
                              ]
                                .filter((_, i) => i !== mIdx)
                                .map((x) => x[0]);
                              setFieldValues((prev) => ({
                                ...prev,
                                [field.id]: [textOnly, ...remaining].filter(Boolean).join('\n'),
                              }));
                            }}
                          >
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }} edges={['top']}>
          ...
          <View style={s.inlineCreateRow}>
            {creatingDeck ? (
              ...
              <TouchableOpacity
                style={s.inlineNewDeckTrigger}
                onPress={() => {
                  setCreatingDeck(true);
                  setTimeout(() => newDeckInputRef.current?.focus(), 100);
                }}
              >
          ...
          <ScrollView contentContainerStyle={{ padding: SPACING.lg }}>
            {decks.map((d) => (
              <TouchableOpacity
                key={d.id}
                style={[s.modalRow, deckId === d.id && s.modalRowActive]}
                onPress={() => {
                  setDeckId(d.id);
                  setDeckChooserOpen(false);
                }}
              >
                <Text style={[s.modalRowText, deckId === d.id && s.modalRowTextActive]}>
                  {d.name}
                </Text>
