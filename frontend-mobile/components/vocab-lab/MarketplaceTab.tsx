...
interface SharedDeck {
  ...
  createdAt: string;
  cardCount: number;
  publisher: { id: string; firstName: string | null; lastName: string | null; avatar: string | null };
}

// ── Constants ──────────────────────────────────────────────────
const CATEGORIES = ['English', 'IELTS', 'TOEFL', 'TOEIC', 'Academic', 'Business', 'Medical', 'Legal', 'Science', 'Daily'];
...
function SharedDeckCard({ deck, onImported }: { deck: SharedDeck; onImported?: () => void }) {
  ...
  const publisherName = [deck.publisher.firstName, deck.publisher.lastName].filter(Boolean).join(' ') || 'Unknown';
  const publishDate = new Date(deck.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  ...
  return (
    <View style={s.deckCard}>
      {/* Name + Description */}
      <Text style={s.deckName} numberOfLines={2}>{deck.name}</Text>
      {deck.description ? (
        <Text style={s.deckDesc} numberOfLines={3}>{deck.description}</Text>
      ) : null}
      ...
      <View style={s.metaRow}>
        <Text style={s.metaText}><Text style={{ fontFamily: 'Farro-Bold' }}>{deck.cardCount}</Text> cards</Text>
      ...
      <View style={s.publisherRow}>
        ...
        {deck.publisher.avatar ? (
          ...
          <View style={[s.publisherAvatar, { backgroundColor: COLORS.primary + '33', alignItems: 'center', justifyContent: 'center' }]}>
            <Text style={{ fontSize: 10, color: COLORS.primary }}>{(deck.publisher.firstName?.[0] || 'U').toUpperCase()}</Text>
          </View>
        )}
        <Text style={s.publisherName} numberOfLines={1}>{publisherName}</Text>
  ...
  const fetchDecks = useCallback(async () => {
    ...
    try {
      ...
      if (pageTab === 'my-published' && user) {
        ...
        data = await vocabLabApi.browseSharedDecks({ search, sort, category: category || undefined });
      }
      setDecks(data);
    } catch { /* silent */ }
    ...
    >
      ...
      <View style={s.tabRow}>
        {(['explore', ...(user ? ['my-published'] : [])] as PageTab[]).map(tab => (
          <TouchableOpacity key={tab} onPress={() => setPageTab(tab)} style={[s.tabBtn, pageTab === tab && s.tabBtnActive]}>
      ...
      {pageTab === 'explore' && (
        <>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.categoryScroll}>
            <TouchableOpacity onPress={() => setCategory('')} style={[s.catChip, category === '' && s.catChipActive]}>
              <Text style={[s.catChipText, category === '' && s.catChipTextActive]}>All</Text>
            </TouchableOpacity>
            {CATEGORIES.map(cat => (
              <TouchableOpacity key={cat} onPress={() => setCategory(cat)} style={[s.catChip, category === cat && s.catChipActive]}>
          ...
          <View style={s.searchRow}>
            ...
            <TouchableOpacity
              ...
              style={s.sortBtn}
            >
              <Ionicons name={sort === 'popular' ? 'trending-up' : 'time-outline'} size={16} color={COLORS.primary} />
      ...
      {loading
        ...
        : decks.length === 0
          ...
          : decks.map(deck => (
            <SharedDeckCard key={deck.id} deck={deck} onImported={fetchDecks} />
          ))
      }
    </ScrollView>
  );
...
const s = StyleSheet.create({
  ...
  tabRow: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#e5e7eb', marginBottom: 20, gap: 0 },
  tabBtn: { paddingVertical: 10, paddingHorizontal: 4, marginRight: 20, borderBottomWidth: 2, borderBottomColor: 'transparent' },
  ...
  catChip: { paddingVertical: 6, paddingHorizontal: 14, borderRadius: 20, backgroundColor: '#f3f4f6' },
  catChipActive: { backgroundColor: COLORS.primary },
  catChipText: { fontSize: 13, color: '#6b7280' },
  catChipTextActive: { color: '#212529' },
  searchRow: { flexDirection: 'row', gap: 10, marginBottom: 16 },
  searchBox: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#fff', borderRadius: 14, paddingHorizontal: 14, paddingVertical: 10, borderWidth: 1, borderColor: '#e5e7eb' },
  searchInput: { flex: 1, fontSize: 14, color: '#212529' },
  sortBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 14, paddingVertical: 10, backgroundColor: COLORS.primary + '18', borderRadius: 14 },
  sortBtnText: { fontSize: 13, color: COLORS.primary },
  // DeckCard
  deckCard: { backgroundColor: '#fff', borderRadius: 16, padding: 16, marginBottom: 14, borderWidth: 1, borderColor: '#f0f0f0', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 },
  ...
  importBtn: { paddingHorizontal: 16, paddingVertical: 8, backgroundColor: COLORS.primary + '18', borderRadius: 10 },
  importBtnDone: { backgroundColor: '#d1fae5' },
  importBtnText: { fontSize: 13, color: COLORS.primary },
  // Empty
  emptyBox: { alignItems: 'center', paddingVertical: 48, backgroundColor: '#fff', borderRadius: 16, borderWidth: 1, borderColor: '#e5e7eb', borderStyle: 'dashed', marginTop: 8 },
