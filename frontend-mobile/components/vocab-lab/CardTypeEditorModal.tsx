...
export function CardTypeEditorModal({
  ...
  const handleAddField = () => {
    ...
    setFields([
      ...fields,
      { id: newId, name: 'New Field', order: fields.length, fieldType: 'text' },
    ]);
  };
  ...
  const toggleFieldInTemplate = (fieldId: string, side: 'front' | 'back') => {
    setTemplates(
      templates.map((t, i) => {
        ...
        const newList = list.includes(fieldId)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const deletedFields = originalFields.filter(
        (of) => !currentFields.find((cf) => cf.id === of.id),
      );
      const addedFields = currentFields.filter((cf) => cf.id.startsWith('field_'));
      const updatedFields = currentFields.filter(
        (cf) =>
          !cf.id.startsWith('field_') &&
          originalFields.find(
            (of) =>
              of.id === cf.id &&
              (of.name !== cf.name || of.order !== cf.order || of.fieldType !== cf.fieldType),
          ),
      );
      ...
      for (const af of addedFields) {
        const res = await vocabLabApi.addField(cardType.id, {
          name: af.name,
      ...
      for (const uf of updatedFields) {
        await vocabLabApi.updateField(cardType.id, uf.id, {
          name: uf.name,
          order: uf.order,
        ...
        >
          ...
          >
                ...
                {fields.map((f, i) => (
                  <View key={f.id} style={s.fieldRow}>
                    <View style={s.fieldControls}>
                      ...
                      <TouchableOpacity
                        onPress={() => handleMoveField(i, 1)}
                        disabled={i === fields.length - 1}
                      >
                        <Ionicons
                          name="chevron-down"
                          size={20}
                          color={i === fields.length - 1 ? COLORS.border : COLORS.textMuted}
                        />
                      </TouchableOpacity>
                    </View>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() =>
                          handleUpdateField(
                            f.id,
                            'fieldType',
                            f.fieldType === 'media' ? 'text' : 'media',
                          )
                        }
                      >
                        <Text style={s.typeToggleText}>
                          {f.fieldType === 'media' ? 'Media' : 'Text'}
                        </Text>
                        <Ionicons
                          name={f.fieldType === 'media' ? 'image-outline' : 'text-outline'}
                          size={14}
            ...
            {tab === 'templates' && (
              <View>
                <Text style={s.sectionDesc}>
                  Select which fields appear on the Front and Back of the card.
                </Text>
                  ...
                  {fields.map((f) => {
                    ...
                    return (
                      <View key={`front_${f.id}`} style={s.tplRowContainer}>
                        <TouchableOpacity
                          style={[s.tplRow, active && s.tplRowActive]}
                          onPress={() => toggleFieldInTemplate(f.id, 'front')}
                        >
                          ...
                          <View style={s.styleRow}>
                            ...
                            >
                              ...
                              <TouchableOpacity
                                style={[
                                  s.styleBtn,
                                  tpl.fieldStyles?.[f.id]?.fontWeight === 'bold' &&
                                    s.styleBtnActive,
                                ]}
                                onPress={() => toggleFieldStyle(f.id, 'fontWeight', 'bold')}
                              >
                              ...
                              <TouchableOpacity
                                style={[
                                  s.styleBtn,
                                  tpl.fieldStyles?.[f.id]?.fontStyle === 'italic' &&
                                    s.styleBtnActive,
                                ]}
                                onPress={() => toggleFieldStyle(f.id, 'fontStyle', 'italic')}
                              >
                              ...
                              <TouchableOpacity
                                style={[
                                  s.styleBtn,
                                  tpl.fieldStyles?.[f.id]?.textDecorationLine === 'underline' &&
                                    s.styleBtnActive,
                                ]}
                                onPress={() =>
                                  toggleFieldStyle(f.id, 'textDecorationLine', 'underline')
                                }
                              >
                                <Text style={[s.styleBtnText, { textDecorationLine: 'underline' }]}>
                                  U
                                </Text>
                              </TouchableOpacity>
                              ...
                              <TouchableOpacity
                                style={[
                                  s.styleBtn,
                                  tpl.fieldStyles?.[f.id]?.textAlign === 'left' && s.styleBtnActive,
                                ]}
                                onPress={() => toggleFieldStyle(f.id, 'textAlign', 'left')}
                              >
                                <Ionicons
                                  name="menu"
                                  size={16}
                                  color={
                                    tpl.fieldStyles?.[f.id]?.textAlign === 'left'
                                      ? '#fff'
                                      : COLORS.text
                                  }
                              ...
                              <TouchableOpacity
                                style={[
                                  s.styleBtn,
                                  tpl.fieldStyles?.[f.id]?.textAlign === 'center' &&
                                    s.styleBtnActive,
                                ]}
                                onPress={() => toggleFieldStyle(f.id, 'textAlign', 'center')}
                              >
                                <Ionicons
                                  name="menu"
                                  size={16}
                              ...
                              <TouchableOpacity
                                style={[
                                  s.styleBtn,
                                  tpl.fieldStyles?.[f.id]?.textAlign === 'right' &&
                                    s.styleBtnActive,
                                ]}
                                onPress={() => toggleFieldStyle(f.id, 'textAlign', 'right')}
                              >
                                <Ionicons
                                  name="menu"
                                  size={16}
                              ...
                              <TouchableOpacity
                                style={s.styleBtn}
                                onPress={() => handleFontSizeSelect(f.id)}
                              >
                                <Text style={s.styleBtnText}>
                                  Size: {tpl.fieldStyles?.[f.id]?.fontSize || 'Default'}{' '}
                  ...
                  {fields.map((f) => {
                    ...
                    return (
                      <View key={`back_${f.id}`} style={s.tplRowContainer}>
                        <TouchableOpacity
                          style={[s.tplRow, active && s.tplRowActive]}
                          onPress={() => toggleFieldInTemplate(f.id, 'back')}
                        >
                          <Text style={[s.tplRowText, active && s.tplRowTextActive]}>{f.name}</Text>
                          <View style={[s.checkbox, active && s.checkboxActive]}>
                            {active && <Ionicons name="checkmark" size={16} color="#fff" />}
                    ...
                    >
                      <TouchableOpacity onPress={() => openColorPicker('backgroundColor')}>
                        <View
                          style={[
                            s.colorSwatch,
                            { backgroundColor: tpl.cardStyle?.backgroundColor || '#ffffff' },
                          ]}
                        />
                      </TouchableOpacity>
                      <TextInput
                    ...
                    >
                      <TouchableOpacity onPress={() => openColorPicker('color')}>
                        <View
                          style={[
                            s.colorSwatch,
                            { backgroundColor: tpl.cardStyle?.color || '#000000' },
                          ]}
                        />
                      </TouchableOpacity>
                      <TextInput
