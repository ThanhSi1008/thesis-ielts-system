...
export function GradingProvider({ children }: { children: ReactNode }) {
  ...
  const patchJob = useCallback((sessionId: string, patch: Partial<GradingJob>) => {
    setJobs((prev) =>
      prev.map((j) => (j.sessionId === sessionId ? { ...j, ...patch } : j))
    );
  }, []);

  const startPolling = useCallback((sessionId: string, examType: 'SPEAKING' | 'WRITING' | 'INTENSIVE', resultUrl: string) => {
  ...
  const submitAndTrack = useCallback(async (params: SubmitAndTrackParams) => {
