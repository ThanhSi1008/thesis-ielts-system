...
| ------ | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| **D1** | Công cụ cào/trích xuất | **Hybrid tối giản**: Scraper thô (Playwright/BeautifulSoup + PyMuPDF) chỉ làm việc _vật lý_ (tải audio/ảnh + lấy `innerText`/text PDF thô) → **Gemini Structured Outputs** cấu trúc hoá → Admin duyệt | Tách bạch 3 giai đoạn; tái dùng Gemini & RabbitMQ sẵn có ở `backend-ai`          |
...
| [`06-ui-testing-scenario.md`](./06-ui-testing-scenario.md)       | **Kịch bản kiểm thử toàn diện trên UI** (Tiếng Việt) — Hướng dẫn chi tiết chạy end-to-end thủ công để test Admin Exam Builder & Student FULL_TEST Player                                                                  | Thử nghiệm thực tế              |
| [`demo-script.md`](./demo-script.md)                             | **Detailed Thesis Demo Script** (English) — Step-by-step presentation/demonstration walkthrough covering Admin Staging Queue, full-screen Review Editor, sequential exam player with auto-resume, and AI grading pipeline | Thesis Presentation / Demo      |

Đọc theo thứ tự `01 → 02 → 03 → 04`; sau khi đã code thì xem `05` để rà lỗi, sử dụng `06` hoặc `07` để chạy thử nghiệm kịch bản demo.
...
| **IELTS Advanced**     | **Ngân hàng luyện tập** chia theo kỹ năng. Mỗi đơn vị là một _Part_ (Listening/Reading) hoặc _Prompt_ (Writing) hoặc _Part_ (Speaking) **độc lập**, không gom thành "đề". |
...
| **ContentImportJob**   | Bảng _staging_ mới, lưu trạng thái 1 lượt import: text thô, JSON nháp do Gemini sinh, media đã tải, để Admin duyệt trước khi commit.                                      |
