...
Gemini được lựa chọn so với các mô hình khác (GPT-4, Claude) vì: cung cấp API với quota phù hợp cho giai đoạn phát triển; tích hợp liền mạch với hệ sinh thái Google Cloud qua thư viện google-genai; và hỗ trợ đầu vào dài cho phép gửi toàn bộ bài viết IELTS trong một lần gọi API mà không bị giới hạn độ dài.
...
Trong hệ thống IELTS, NestJS Backend Core triển khai cơ chế xác thực JWT hai token:

\begin{itemize}
    \item \textbf{Access token:} Thời hạn 7 ngày, gửi kèm mỗi API request trong Authorization header. NestJS sử dụng thư viện @nestjs/jwt và passport-jwt để xác thực token và inject thông tin người dùng qua decorator @CurrentUser().
    \item \textbf{Refresh token:} Thời hạn 30 ngày, lưu phía client (AsyncStorage trên mobile, localStorage trên web), dùng để cấp lại access token khi hết hạn mà không yêu cầu đăng nhập lại.
\end{itemize}
