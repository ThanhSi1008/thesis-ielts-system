...
Về bảo mật, hệ thống áp dụng đồng thời nhiều lớp bảo vệ: mật khẩu được băm bằng bcrypt với 10 vòng tăng cường (\texttt{saltRounds = 10}), xác thực bằng JWT access token có hiệu lực 7 ngày, và giới hạn tốc độ truy cập (rate limiting) toàn cục 100 request/60 giây để ngăn chặn brute-force. Prisma ORM~\cite{prisma-docs} ngăn chặn SQL injection qua parameterized queries và cung cấp type-safe database access nhất quán trên toàn backend.
