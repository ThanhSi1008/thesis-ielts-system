...
Đồ án hoàn thiện hệ thống IELTS Master English (Lexon) với kiến trúc Event-driven Modular Monolith trên các công nghệ hiện đại, bám sát chuẩn phát triển phần mềm cấp sản xuất.

Về kiến trúc, hệ thống tổ chức theo mô hình Event-driven Modular Monolith. NestJS Backend Core gói 18 module nghiệp vụ (auth, exams, vocab-lab, grammar, shadowing, gamification, subscriptions\ldots) trong một ứng dụng duy nhất, giao tiếp nội bộ qua Dependency Injection --- giữ tính nhất quán và dễ bảo trì. FastAPI AI Worker tách thành microservice độc lập, kết nối với core qua RabbitMQ để chạy AI inference bất đồng bộ mà không làm chậm phản hồi API. Cách bố trí này cân bằng giữa độ đơn giản của monolith và khả năng scale độc lập cho phần tính toán nặng (AI grading).

Về đa nền tảng, hệ thống đẩy ra song song giao diện web (Next.js với Server-Side Rendering) và ứng dụng di động (mobile) (React Native), dùng chung một backend API. Next.js giúp tối ưu SEO và tốc độ tải trang qua SSR và Static Site Generation; React Native cho phép deploy iOS và Android từ một codebase TypeScript duy nhất, cắt đáng kể chi phí so với native riêng biệt~\cite{reactnative-docs}.

Về tích hợp trí tuệ nhân tạo, hệ thống đưa Gemini API~\cite{gemini-api} (model gemini-2.5-flash) vào pipeline chấm tự động bài Writing theo bốn tiêu chí IELTS chính thức (Task Achievement, Coherence and Cohesion, Lexical Resource, Grammatical Range and Accuracy) và bài Speaking theo bốn tiêu chí Speaking tương ứng. faster-whisper~\cite{whisper-paper} --- bản tối ưu hóa của OpenAI Whisper trên CTranslate2 backend --- chạy cục bộ trong Docker container, đảm nhiệm chuyển giọng nói thành văn bản cho chấm phát âm và dictation; hoàn toàn tự chủ, không chịu quota từ dịch vụ bên thứ ba. Pipeline grading bất đồng bộ qua RabbitMQ giữ cho API trả về tức thì dù inference mất vài giây.

Về bảo mật, hệ thống đặt nhiều lớp phòng thủ song song: mật khẩu băm bằng bcrypt với 10 vòng (saltRounds = 10), xác thực qua JWT access token hạn 7 ngày, rate limiting toàn cục 100 request/60 giây chặn brute-force. Prisma ORM~\cite{prisma-docs} ngăn SQL injection thông qua parameterized queries và cung cấp truy cập DB type-safe nhất quán trên toàn backend.

\subsection{Về mặt chức năng nghiệp vụ}

Hệ thống đã hiện thực đủ các chức năng cốt lõi của một nền tảng luyện thi IELTS toàn diện, phục vụ người học từ band 4.0 đến 7.5+ theo chuẩn IELTS quốc tế~\cite{ielts-global-stats}:
...
Hệ thống vẫn để lộ một số hạn chế cần thừa nhận thẳng thắn:
...
\begin{itemize}
    ...
    \item \textbf{Chưa hỗ trợ chế độ offline:} Ứng dụng di động (mobile) yêu cầu kết nối Internet liên tục; nội dung bài tập được phục vụ qua REST API (từ cơ sở dữ liệu PostgreSQL) và file audio được tải từ Cloudinary, chưa có cơ chế cache cục bộ cho thiết bị.
...
Trên nền tảng đã có và các hạn chế vừa nêu, một số hướng phát triển tiếp theo được đề xuất cho các giai đoạn sau:

\begin{itemize}
    \item \textbf{Fine-tune mô hình AI chuyên biệt:} Xây dựng tập dữ liệu bài mẫu IELTS đã được giám khảo chấm điểm thực tế để fine-tune mô hình ngôn ngữ, nhằm tăng độ chính xác AI scoring và đưa chênh lệch band điểm so với giám khảo con người xuống mức $\leq$0.5~\cite{nguyen2026aiwriting}.
    ...
    \item \textbf{Adaptive Learning:} Dựa trên lịch sử kết quả làm bài và phân tích lỗi thường gặp, hệ thống tự động gợi ý bài tập phù hợp với điểm yếu của từng người học thay vì để người học tự chọn, tạo lộ trình học tập cá nhân hoá thực sự~\cite{nguyen2024aiteaching}.

    \item \textbf{Chế độ offline cho di động (mobile):} Tích hợp cơ chế download bài tập và cache audio cục bộ để người học có thể luyện tập khi không có Internet, đồng bộ kết quả lên server khi kết nối trở lại thông qua conflict resolution strategy.
    ...
    \item \textbf{Triển khai chính thức trên iOS và App Store:} Hoàn thiện kiểm thử trên thiết bị iOS, đăng ký Apple Developer Program và phân phối ứng dụng trên App Store nhằm tiếp cận phân khúc người dùng iPhone -- nhóm chiếm tỷ lệ lớn trong tệp người học IELTS.
\end{itemize}
