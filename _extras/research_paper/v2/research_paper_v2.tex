...
\begin{abstract}
Automated assessment of subjective English language skills remains an open challenge in computer-assisted language learning. This paper presents IELTS Master English AI, a comprehensive IELTS preparation platform built on an event-driven hybrid architecture that separates AI inference from core application logic via RabbitMQ message brokering. The platform implements three principal techniques: (1)~LLM-based writing and speaking grading via Gemini 2.5 Flash with structured JSON rubric output aligned to official IELTS band descriptors, (2)~multimodal speaking assessment combining Faster-Whisper speech-to-text transcription with direct audio submission to Gemini for pronunciation and fluency evaluation, and (3)~an FSRS-based spaced repetition vocabulary system with adaptive, bounded interval scheduling. Preliminary validation on $N=30$ human-scored IELTS Task~2 essays demonstrates strong inter-rater agreement between the LLM grader and human examiners, achieving Pearson $r=0.97$, Cohen's $\kappa=0.96$, and 96.7\% of predicted scores within $\pm$0.5 bands of the human reference.
\end{abstract}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth,keepaspectratio]{figure_1_system_architecture_overview.png}}
...
\begin{equation}
\text{Overall} = \text{round}_{0.5}\!\left(\frac{B_{\text{Task1}} + B_{\text{Task2}} \times 2}{3}\right)
\label{eq:overall}
\end{equation}
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{pronunciation_multi_metric_boxplot.png}}
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{ipa_vs_levenshtein_comparison.png}}
